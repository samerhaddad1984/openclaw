#!/usr/bin/env python3
"""
setup_cloudflare.py — OtoCPA: Cloudflare Tunnel setup wizard
====================================================================
Interactive script that:
  ...
  6. Writes the public HTTPS URL to otocpa.config.json as
     "public_portal_url".
...
def step_create_tunnel(cloudflared: Path, cf_dir: Path) -> str:
    """Create (or reuse) the 'otocpa' tunnel.  Returns the tunnel UUID."""
    ...
    if not tunnel_id:
        ...
        print(result.stdout)
        # Extract UUID from output like "Created tunnel otocpa with id <uuid>"
...
def step_write_config(cf_dir: Path, tunnel_id: str, install_dir: Path) -> Path:
    ...
    config_content = f"""\
# cloudflared tunnel config — generated by OtoCPA setup_cloudflare.py
tunnel: {tunnel_id}
...
# ──────────────────────────────────────────────────────────────────────────────
# Step 7 — Save public URL to otocpa.config.json
# ──────────────────────────────────────────────────────────────────────────────

def step_save_url(public_url: str) -> None:
    _print_step(7, "Save public URL to otocpa.config.json")
    cfg = _load_config()
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="OtoCPA — Cloudflare Tunnel setup wizard",
    )
    parser.add_argument(
        "--install-dir",
        default=str(ROOT),
    ...
    print()
    print("═" * 60)
    print("  OtoCPA — Cloudflare Tunnel Setup")
    print("═" * 60)
    ...
    try:
        ...
        print()
        print("  The OtoCPA client portal is now accessible at:")
